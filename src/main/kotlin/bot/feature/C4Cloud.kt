    ...
    init {
        ...
        jda.addEventListener(object : ListenerAdapter() {
            ...
            override fun onSlashCommandInteraction(event: SlashCommandInteractionEvent) {
                if (event.name != "c4vxl-cloud") return

                when(event.subcommandName?.lowercase(Locale.getDefault())) {
                    "connect" -> {
                        val apiKey: String = event.getOption("api_key", OptionMapping::getAsString) ?: return
                    ...
                    "editor" -> {
                        val path: String = event.getOption("path", OptionMapping::getAsString) ?: return
                        val user = event.user.asCloudConnectedUser
